...
@property (nonatomic, strong)UIView *tempBar;
@end

@implementation SlideViewController
- (UIView *)tempBar {
    if (!_tempBar) {
        _tempBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, WY_TopBar_H)];
        _tempBar.backgroundColor = [UIColor whiteColor];
    }
    return _tempBar;
}
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        _coverButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, WY_ScreenWidth, WY_ScreenHeight)];
...
- (void)pan:(UIPanGestureRecognizer*)pan{
    ...
    if (_panGestureRecognizer.state==UIGestureRecognizerStateEnded || _panGestureRecognizer.state == UIGestureRecognizerStateCancelled) {
        if (_currentView.frame.origin.x==0) {
            [self showShadow:false];
        }else{
...
#pragma clang diagnostic pop
    ...
    if (xoffset == 0) {
        [self.tempBar removeFromSuperview];
    }else {
        if (![_currentView.subviews containsObject:self.tempBar]) {
            [_currentView insertSubview:self.tempBar atIndex:0];
        }
...
- (BOOL)shouldAutorotate {
    if (WY_IsKindOfClass(self.rootViewController, WYBaseNVC)) {
        return ((WYBaseNVC *)self.rootViewController).shouldAutorotate;
    }
    return NO;
}
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations {
    if (WY_IsKindOfClass(self.rootViewController, WYBaseNVC)) {
        return ((WYBaseNVC *)self.rootViewController).supportedInterfaceOrientations;
    }
