...
/**
 Close Device push(关闭设备推送)
 
 @param deviceID 设备ID
 @param device  ID 对应的设备 camera（ The device camera corresponding to the ID）
 @param closePush 是否关闭推送 YES 关闭推送 NO 不关闭推送（Whether to close the push YES to close the push NO to not close the push）
 @param success Successful callback (成功回调)
 @param failure failure callback (失败回调)
 */
- (void)deviceAlarmNotificationClosePush:(BOOL)closePush camera:(MeariDevice *)device deviceID:(NSInteger)deviceID success:(MeariSuccess)success failure:(MeariFailure)failure;
