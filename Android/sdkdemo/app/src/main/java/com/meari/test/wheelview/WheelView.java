...
public class WheelView extends View {
	...
	/** Label offset ,   dip */
	private static int LABEL_OFFSET_DIP = 16;
	/** Label offset */
	private static int LABEL_OFFSET = 24;
...
//		setBackgroundResource(R.drawable.wheel_bg);
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (mode == MeasureSpec.EXACTLY) {
			...
			width = itemsWidth + labelWidth + PADDINGRIGHT +PADDINGLEFT;
			if (labelWidth > 0) {
				width += LABEL_OFFSET;
			}
		...
		if (recalculate) {
			// recalculate width
			int pureWidth = width - LABEL_OFFSET - PADDINGRIGHT - PADDINGLEFT;
			if (pureWidth <= 0) {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsWidth > 0) {
			...
			canvas.translate(PADDINGLEFT, -ITEM_OFFSET);
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
