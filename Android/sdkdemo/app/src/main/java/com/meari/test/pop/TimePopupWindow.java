...
import com.meari.test.wheelview.WheelTime;
...
public class TimePopupWindow extends PopupWindow implements OnClickListener {
	...
	public TimePopupWindow(Context context, Type type) {
		...
		rootView = mLayoutInflater.inflate(R.layout.pop_pick_time, null);
		// -----确定和取消按钮
	 ...
	 * @param date
	 */
	public void showAtLocationByTime(View parent, int gravity, int x, int y, Date date, int hours, int minute ) {
		Calendar calendar = Calendar.getInstance();
		if (date == null)
			calendar.setTimeInMillis(System.currentTimeMillis());
		else
