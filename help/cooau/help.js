...
(function () {
    ...
    var open = function open(s) {
        return [m(".more-name.open" + fontFit(s.name), { onclick: function onclick() {
                s.open = false;
            } }, m.trust('<p>' + s.name)), content(s)];
    };
    var close = function close(s) {
        return [m(".more-name.close" + fontFit(s.name), { onclick: function onclick() {
                s.open = true;
            } }, m.trust('<p>' + s.name))];
    };
    var Item = {},
    ...
    var fontFit = function fontFit(s) {
        return !en && s.replace(/[\u0391-\uFFE5]/g, "11").length < 41 ? "" : ".font-10";
    };
    ...
    window.help = {
        ...
        more: function more(data) {
            var resolved = en ? "Resolved" : (window.location.href.indexOf("ja") != -1 ? "解決" : "已解决");
            var zanPushed = false;
            /*var zan = function zan() {
                return !zanPushed ? m(".zan", { onclick: function onclick() {
                        zanPushed = true;
